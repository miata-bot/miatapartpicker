defmodule Partpicker.TCGTest do
  ...
  test "trade accept" do
    ...
    [user1, user2] = [Repo.reload!(user1), Repo.reload!(user2)] |> Repo.preload([:cards])
    assert Enum.find(user1.cards, fn %{id: id} -> id == virtual2.id end)
    assert Enum.find(user2.cards, fn %{id: id} -> id == virtual1.id end)
  end
end
